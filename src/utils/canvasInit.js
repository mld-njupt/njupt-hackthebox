...
var ParticleWave = function () {
  ...
  var initSmoothGradient = function () {
    ...
    smoothGradient.addColorStop(0.51, "rgba(0, 0, 0, 0.9)");
    smoothGradient.addColorStop(0.52, "rgba(0, 0, 0, 1)");
  ...
  var renderParticle = function () {
    ...
    for (var i = 0, xIndex, zIndex; i < particle.length; i += 2) {
      ...
      p.y +=
        (Math.sin(particle[xIndex] * 50 + particleWaveWalker) * addY) /
        waveControl;
      p.y +=
        (Math.cos(particle[zIndex] * 10 + particleWaveWalker) * addY) /
        waveControl;

      p.y -=
        (Math.cos(
          particle[zIndex] + particle[xIndex] * 10 + particleWaveWalker
        ) *
          addY) /
        waveControl;

      p.y -=
        (Math.cos(particle[xIndex] * 50 + particleWaveWalker) * addY) /
        waveControl;
      p.y -=
        (Math.sin(particle[zIndex] * 10 + particleWaveWalker) * addY) /
        waveControl;
